...
const ProductDetails = () => {
  ...
  const mainQuantity = parseInt(
    product.mainQuantity?.$numberInt || product.mainQuantity
  );
  const price = parseFloat(
    product.price?.$numberDouble || product.price
  );
  const rating = parseFloat(
    product.rating?.$numberDouble || product.rating
  );
  ...
  const handleBuy = () => {
    ...
    fetch(`http://localhost:3000/products/${objectId}`, {
      ...
      .then((data) => {
        Swal.fire(
          "Congratulations!",
          `You have successfully purchased ${quantity} units.`,
          "success"
        );

        // Update local state with new mainQuantity
        setProduct((prev) => ({
          ...prev,
